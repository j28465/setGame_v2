<template>
  ...
  <div id="park">
    ...
    <div id="tableBoard" ref="tableBoard">
      <div id="tableBlock">
        <Card v-for="c in tableTopCards" :key="c" :v="c.join(',')" @chooseCard="choose"></Card>
      </div>
    </div>
  </div>
</template>
...
const dealCards = (index: number) => {
  for (let i = 0; i < index; ++i) {
    tableTopCards.value.push(lib.cards.pop() as number[]);
  }
};
//清空或在同位置放上新的牌
const changeCard = (index: number) => {
  let c = lib.cards.pop();
  tableTopCards.value[index] = c ? c : [];
};
//移除這張牌
const removeCard = (index: number) => {
  tableTopCards.value.splice(index, 1);
};
//符合答案的提示牌組
    ...
    setTimeout(() => {
      ...
      document.querySelectorAll('#tableBoard .card').forEach((e, i) => {
        if (e.classList.contains('lock')) {
          if (tableTopCards.value.length > 12) {
            //刪除外框
            e.remove();
            //清除現在檯面上被選走的牌
            ...
            //清空或在同位置放上新的牌
            changeCard(i);
            let len: number[][] = tableTopCards.value.filter((v: number[]) => v.length != 0);
...
const hintClick = (): void => {
  if (hintCards.value.length > 0) {
    ...
    let jdg = false;
    //產生一組解答
    let len = tableTopCards.value.length;
    for (let t1 = 0; t1 < len; ++t1) {
      if (tableTopCards.value[t1].length > 0) {
        for (let t2 = t1 + 1; t2 < len; ++t2) {
          if (tableTopCards.value[t2].length > 0) {
            for (let t3 = t2 + 1; t3 < len; ++t3) {
              if (tableTopCards.value[t3].length > 0) {
                let tmp: number[][] = [tableTopCards.value[t1], tableTopCards.value[t2], tableTopCards.value[t3]];
                jdg = verify(tmp);
